...
const DeploymentConfig = require('./deploymentConfig.js')
...
class Settings {
  ...
  constructor (nop, context, repo, config, ref, suborg) {
    ...
    this.results = []
    this.errors = []
    this.settings = new DeploymentConfig()
    this.mergeDeep = new MergeDeep(this.log, this.github, [], this.settings)
  }
  ...
  async createCheckRun () {
    ...
    if (this.errors.length > 0) {
      ...
      details = eta.renderString(errorTemplate, this.errors)
    }
...
`
  ...
  async updateRepos (repo) {
    ...
    try {
      const pluginConfigs = await this.childPluginsList(repo)
      this.log.debug(`Plugin Configs: ${JSON.stringify(pluginConfigs, null, 2)}`)

      const plugins = await Promise.all(pluginConfigs.map(([Plugin, config]) =>
        new Plugin(this.nop, this.github, repo, config, this.settings, this.log, this.errors).sync()
      ))

      this.appendToResults(plugins)
    } catch (e) {
      if (this.nop) {
  ...
  async updateAll () {
    ...
    return this.eachRepositoryRepos(this.github, this.settings.restrictedRepos).then(res => {
      this.log.debug(res)
      this.appendToResults(res)
    })
  }
  ...
  async getUnsafeRepoConfig (repo) {
    const unsafeSettings = await ConfigManager.loadYaml(this.github, {
      ...repo,
      path: '.github/settings.yml'
    })

    const { unsafeFields } = this.settings

    const result = {}
    for (const unsafeField of unsafeFields) {
      let value
      try {
        value = jsonPointer.get(unsafeSettings, unsafeField)
      } catch {}
  ...
  validate (section, baseConfig, overrideConfig) {
    const configValidator = this.settings.configValidators[section]
    if (configValidator) {
    ...
    const overridevalidator = this.settings.overrideValidators[section]
  ...
  isRestricted (repoName) {
    const restrictedRepos = this.settings.restrictedRepos
    ...
    if (Array.isArray(restrictedRepos)) {
      ...
      if (restrictedRepos.includes(repoName)) {
        this.log.debug(`Skipping restricted repo ${repoName}`)
        return true
      } else {
